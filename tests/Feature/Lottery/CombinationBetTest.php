...
it('pode vincular uma combinação ao concurso atual', function () {
    ...
    foreach ([19, 46, 69, 70, 79] as $number) {
    ...
    $response = $this->postJson("/lottery/modalities/{$quina->id}/combination-history/{$history->id}/register-bet");
...
it('pode conferir a aposta com o resultado oficial', function () {
    ...
    $history = CombinationHistory::factory()->create([
        'lottery_modality_id' => $quina->id,
        'numbers' => [19, 22, 49, 56, 79],
        'bet_contest_number' => 6999,
        'bet_registered_at' => now(),
    ]);

    $response = $this->get("/lottery/modalities/{$quina->id}/combination-history/{$history->id}/check-bet");
    ...
    $response->assertInertia(fn (Assert $page) => $page
        ...
        ->where('officialResult.hit_count', 2)
        ->where('officialResult.hits', [19, 79])
    );

    expect($history->fresh()->bet_result_snapshot['hit_count'])->toBe(2)
        ->and($history->fresh()->bet_checked_at)->not->toBeNull();
});
